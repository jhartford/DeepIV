...
def monte_carlo_error(g_hat, data_fn, ntest=5000, has_latent=False, debug=False, results_file=None):
...
def demand(n, seed=1, ynoise=1., pnoise=1., ypcor=0.8, use_images=False, test=False):
    ...
    def g(x, z, p): return storeg(x, p)  # doesn't use z
...
def linear_data(n, seed=None, sig_d=0.5, sig_y=2, sig_t=1.5,
    ...
    rng = np.random.RandomState(seed)
    def nox(z, d): return z + 2*d
    def house_price(alpha, d, nox_val): return alpha + 4*d + 2*nox_val
